...
def init_motor():
    motors = [RPWM1, LPWM1, L_EN1, R_EN1, RPWM2, LPWM2, L_EN2, R_EN2]
    for pin in motors:
...
def stop_motor(pwms=[]):
    ...
    all_enable_pins = [L_EN1, R_EN1, L_EN2, R_EN2]
...
if __name__ == "__main__":
    try:
        ...
        while True:
            command = input("Enter command (w: forward, s: backward, q: quit): ").lower()
            if command == 'w':
                move_forward()
            elif command == 's':
                move_backward()
